...
module.exports = {
    viewOne: function(req, res, next){
        ...
        func.oneRecipe(req.params.id)
          ...
          .catch(function(err)   {console.log('Error en viewOne : ' + err.message)});
    },
    viewRecipeUser: function(req, res, next) {
        ...
        func.findRecipe('ownerRecipe', req.params.userLogin)
          ...
          .catch(function(err)   {console.log('Error en viewRecipeUser : ' + err.message)});
    },
    ...
    creIngr: function (req, res, next) {
        ...
        Recipes.update( { _id: req.params.id }, {$push : { ingredients : req.body } }, function(err, result) {
            if(err) {
                ...
                func.oneRecipe(req.params.id)
                  .then(function(resultRecipe) {return res.json(resultRecipe)})
                  .catch(function(err)         {console.log('Error en creIngr : ' + err.message)});
            }
        });
    },
        ...
        Recipes.findOneAndUpdate({ "_id"             : req.params.idRecipe,
            ...
            if(err) {
                ...
                func.oneRecipe(req.params.idRecipe)
                  .then(function(resultRecipe) {return res.json(resultRecipe)})
                  .catch(function(err)         {console.log('Error en costIngr : ' +  err.message)});
            }
        });
    },
        ...
        Recipes.update({ _id             : req.params.idRecipe },
                         { $pull : {ingredients: { _id : req.params.idIngre }}
                           ...
                           if(err) {
                               ...
                               func.oneRecipe(req.params.idRecipe)
                                 .then(function(resultRecipe) {console.log('resultRecipe : ' + resultRecipe);
                                                               return res.json(resultRecipe)})
                                 .catch(function(err)         {console.log('Error en costIngr : ' +  err.message)});
                           }
                       });
                          ...
                          { $group : {
                              ...
                              if (err) {
                                  ...
                                  func.oneRecipeAndUpd(req.params.id, 'cost', resTotCost[0].totalCost)
                                    .then(function(resultRecipe) {console.log('resultRecipe : ' + JSON.stringify(resultRecipe));
                                                                  return res.json(resultRecipe)})
                                    .catch(function(err)         {console.log('Error en calcRecipe : ' +  err.message)});
                              }
                          });
    }
};
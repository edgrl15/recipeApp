angular.module('myRecetaApp')
  .controller('optCaltRecController', [ '$scope', 'LoginFactory', 'RecetaFactory', 
      ...
      return LoginFactory.userLog;
        ...
        if ( LoginFactory.userLog == "" ) {
          ...
          $scope.isUserConnected = true;
           RecetaFactory.findRecipeOwner(LoginFactory.userLog)
    ...
    $scope.$watch('princTempl', function(NewValue, OldValue) {
      $scope.currentTab    = $scope.tabs[0].template;
      $scope.nameBack      = $scope.tabs[0].backTemplName;
      $scope.nameFoward    = $scope.tabs[0].fowardTemplName;
    });
    
    
    $scope.enableRecipe = function() {
